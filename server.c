...
void serverTask(int nsd){
	int select,type,accType,currUserID,mssgLength,option;
	bool result;
	while(1){
		...
		if(option==1){
			...
			accType=1;
			mssgLength=read(nsd,&currUser1,sizeof(normalUser));
			printf("Username: %d\n",currUser1.userID);
			printf("Password: %s\n",currUser1.password);
			...
			jointUser currUser2;
			accType=2;
			mssgLength=read(nsd,&currUser2,sizeof(jointUser));
			...
			admin currUser3;
			accType=3;
			mssgLength=read(nsd,&currUser3,sizeof(admin));
